#!/bin/bash
# RegionRead composition, parameterized by model — for the capability curve.
#   regionread_curve.sh <model_name> <llm_port> <prefix> [nseed]
# baseline (agent chains TextToBbox->RegionAttributeDescription) vs composed
# (RegionRead macro). 8 chain-tasks, proxy #2 (16282 -> tool server 16182 w/ RegionRead).
set -uo pipefail
...
MODEL=$1; PORT=$2; PFX=$3; NSEED=${4:-5}
...
for s in $(seq 1 $NSEED); do
  doeval baseline_prim  ${PFX}_baseline_s$s
  doeval composed_rr    ${PFX}_composed_s$s
done
echo "[$PFX] complete"
